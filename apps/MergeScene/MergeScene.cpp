...
int main(int argc, LPCTSTR* argv)
{
  ...
  for (uint32_t i = 0; i < OPT::gridHeight; i++)
  {
    ...
    {
      uint32_t idx = i*OPT::gridWidth + j;
      subScenes[idx].reset(new Scene());
      if (!subScenes[idx]->Load(MAKE_PATH_SAFE(Util::getFullFileName(OPT::strOutputFileName) +
      "_" + std::to_string(i) + "_" + std::to_string(j) + ".mvs")))
      {
        std::cout << "Failed to load tile " << i << " " << j << std::endl;
        continue;
      }

      if (!scene)
      {
        scene.reset(new Scene(*subScenes[idx]));
      }
      else
      {
	scene->mesh.vertices.Join(subScenes[idx]->mesh.vertices);
	scene->mesh.faces.Join(subScenes[idx]->mesh.faces);
	scene->mesh.vertexNormals.Join(subScenes[idx]->mesh.vertexNormals);
	scene->mesh.vertexFaces.Join(subScenes[idx]->mesh.vertexFaces);
	scene->mesh.vertexBoundary.Join(subScenes[idx]->mesh.vertexBoundary);
	scene->mesh.faceNormals.Join(subScenes[idx]->mesh.faceNormals);
	scene->mesh.faceTexcoords.Join(subScenes[idx]->mesh.faceTexcoords);
      }
    }
  }
